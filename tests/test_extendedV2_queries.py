...
import queries_functions.extendedV2_queries as sq
...
class TestAdditionalFunctions(unittest.TestCase):
    ...
    def test_filter_question_extendedV2(self):
        sq.filter_questions_extendedV2(self.database, q1="Slower aggregation", q2="No", q3="No information", filename="test_filter_1",
                                     directory="../tests/test_json_data")
        path = pl.Path("../tests/test_json_data/test_filter_1.json")
        self.assertEqual((str(path), path.is_file()), (str(path), True))

        sq.filter_questions_extendedV2(self.database, q1="Slower aggregation", q3="No information",
                                     filename="test_filter_2",
                                     directory="../tests/test_json_data")
        path = pl.Path("../tests/test_json_data/test_filter_2.json")
        self.assertEqual((str(path), path.is_file()), (str(path), True))

        sq.filter_questions_extendedV2(self.database, filename="test_filter_3", directory="../tests/test_json_data")
        path = pl.Path("../tests/test_json_data/test_filter_3.json")
        self.assertEqual((str(path), path.is_file()), (str(path), True))

    def test_contains_fragment(self):
        sq.contains_fragment_extendedV2(self.database, "DAEFRHDSG", filename="test_contains_1",
                                      directory="../tests/test_json_data")
        path = pl.Path("../tests/test_json_data/test_contains_1.json")
        self.assertEqual((str(path), path.is_file()), (str(path), True))

    def test_search_phrase(self):
        sq.search_phrase_extendedV2(self.database, "pH", filename="test_phrase_1",
                                  directory="../tests/test_json_data")
